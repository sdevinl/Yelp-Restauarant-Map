...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    features = [feature_fn for feature_fn in feature_fns]
    max_feature = max(features, key= lambda x: find_predictor(user, reviewed, x)[1])
    max_predictor = (find_predictor(user,reviewed,max_feature))[0]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            rest_dct[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            rest_dct[restaurant_name(restaurant)] = predictor(restaurant)
    return rest_dct
    # END Question 9
